...
class _DataManagementScreenState extends ConsumerState<DataManagementScreen> {
  ...
  Future<void> _saveSupabaseSettings() async {
    ...
    try {
      ...
      if (mounted) {
        _showSnack('Supabase 설정 저장에 실패했습니다: ${sanitizeSupabaseError('$e')}');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _uploadSupabaseBackup() async {
    ...
    try {
      ...
      if (mounted) {
        _showSnack('Supabase 백업에 실패했습니다: ${sanitizeSupabaseError('$e')}');
      }
    } finally {
      if (mounted) setState(() => _busy = false);
    }
  ...
  Future<void> _restoreSupabaseBackup() async {
    ...
    try {
      ...
      await ref
          ...
          .recordStorageFailure(e.toString());
      if (mounted) {
        _showSnack('Supabase 복원에 실패했습니다: ${sanitizeSupabaseError('$e')}');
      }
    } finally {
      if (mounted) setState(() => _busy = false);
    }
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                FilledButton.icon(
                  key: const ValueKey('settings-supabase-sync-now-button'),
...
class _AutoSyncStatusPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (status.fullSyncError != null) {
      lines.add('오류: ${status.fullSyncError}');
    }
    ...
    return _StatusPanel(
      panelKey: const ValueKey('settings-supabase-status-panel'),
      title: '자동 동기화',
      badge: badge,
      lines: lines,
    );
...
class _JsonBackupStatusPanel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final badge = _storageBadge(settings, status);
    final lines = <String>[
    ...
    return _StatusPanel(
      panelKey: const ValueKey('settings-json-backup-status-panel'),
      title: 'JSON 백업 / 복원',
      badge: badge,
      lines: lines,
    );
...
class _StatusChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final color = switch (tone) {
      _StatusTone.success => context.desktopIncome,
      _StatusTone.error => context.desktopExpense,
      _StatusTone.progress => context.desktopAccent,
      _StatusTone.warning => Theme.of(context).colorScheme.tertiary,
      _StatusTone.neutral => context.desktopMuted,
    };
    return Container(
